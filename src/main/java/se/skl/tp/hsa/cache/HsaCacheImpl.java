...
public class HsaCacheImpl implements HsaCache {
	...
	/**
	 * Date of creation(latest) from hsa xml files
	 */
	private Date hsaFileCreationDate;
	...
	private Map<String,HsaNode> doInitialize(String ... filenames) throws XMLStreamException, IOException {
		...
		for(String filename: filenames){
			...
			try {
				Date creationDateInFile = df.parse(data.getHsaFileCreationDate());
				if(latestCreationDate == null || creationDateInFile.after(latestCreationDate)){
					latestCreationDate = creationDateInFile;
				}
		...
		hsaFileCreationDate = latestCreationDate;
	 ...
	 */
	@Override
	public Date getHsaFileCreationDate() {
		return hsaFileCreationDate;
	}
